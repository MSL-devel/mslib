...
void SurfaceAreaAndVolume::computeSurfaceAreaAndVolumeStereographicProjectIntegration(){
	/*
	  AVRO algorithm
	 */
...
void SurfaceAreaAndVolume::computeSurfaceAreaAndVolumeStereographicProjectIntegration(AtomPointerVector &_atoms, vector<double> &_radii){
	...
	*****************************************************/
	...
	/****************************************************
             ...
             STEP 3:  ROTATE MOLECULE SO ALL "NORTH POLE" POINTS
                 ARE NOT ON SURFACE POINTS OF ANOTHER ATOM
	...
	*****************************************************/
	...
	while (northPoleInside){
		...
		for (uint i = 0; i < _atoms.size();i++){
			...
			northPole[2] += _radii[i];
			for (uint j = 0; j < collidingSpheres[i].size();j++){

				
				double dist = northPole.distance(_atoms[collidingSpheres[i][j]]->getCoor());

				if (abs(dist -  _radii[collidingSpheres[i][j]]) < 0.0001){
					if (debug){
						cout << "DIST of NP["<<i<<"] to atom["<<collidingSpheres[i][j]<<"]: "<<dist<<" and RADII: "<<_radii[collidingSpheres[i][j]]<<endl;
						cout << _atoms(i)<<endl<<_atoms(collidingSpheres[i][j])<<endl<<"RADII: "<<_radii[i]<<" "<<_radii[collidingSpheres[i][j]]<<endl;
					}
					minDist = dist;
				}
	...
	/****************************************************
             STEP 4:  STEREOGRAPHIC PROJECTION AND
                         CREATION OF CIRCLES

	*****************************************************/
	if (debug){
		cout << endl<<"-->  STEREOGRHAPIC CIRCLE PROJECTIONS OF COLLIDING SPHERES  <--"<<endl<<endl;
	}
	tsCircles.clear();
	...
	for (uint i = 0; i < _atoms.size();i++){
		...
		for (uint j = 0; j < collidingSpheres[i].size();j++){
			...
			if (debug){
				fprintf(stdout,"\tChecking collision with atom %4d; %8.3f , %8.3f\n\n",collidingSpheres[i][j],_radii[i],_radii[collidingSpheres[i][j]]);
			}
			...
			double dx = a1[0] - a2[0];
			double dy = a1[1] - a2[1];
			double zp  = a1[2] + _radii[i] - a2[2];
			double zm  = a1[2] - _radii[i] - a2[2];
			double a  = dx*dx + dy*dy + zp*zp - _radii[collidingSpheres[i][j]]*_radii[collidingSpheres[i][j]];
			double b  = 8*_radii[i]*_radii[i]*dx;
			double c  = 8*_radii[i]*_radii[i]*dy;
			double d  = 4*_radii[i]*_radii[i]* ( dx*dx + dy *dy + zm*zm - _radii[collidingSpheres[i][j]]*_radii[collidingSpheres[i][j]]);
	...
	/****************************************************
             STEP 5: FIND INTERSECTIONS OF CIRCLES 

	*****************************************************/

	// Create arcs from circles and intersections
	if (debug){
		cout << endl<<"-->  FIND INTERSECTIONS  <--"<<endl<<endl;
	}

	// Store atom,circle,intersection angles
	...
	/****************************************************
             STEP 6: DEFINE UNIQUE, NON-HIDDEN ARCS 
                          FROM INTESECTION POINTS

	*****************************************************/

	// For each circle, sort the intersection points.
	// After sorting, find each intersection point that is not inside any other circle.
	if (debug){
		cout << endl<<"-->  FIND ARCS  <--"<<endl<<endl;
	}
	arcs.clear();
	...
	for (uint i = 0; i < _atoms.size();i++){
		...
		// 
		for (uint j = 0; j < tsCircles[i].size();j++){
			if (intersectionAngles[i][j].size() == 0) continue;
			...
			for (uint k = 0; k < intersectionAngles[i][j].size();k++){
				...
				for (uint jj = 0 ; jj < tsCircles[i].size();jj++){
					...
					if (k != 0){
						double tmid = t0 + r0 * cos( (intersectionAngles[i][j][k]+intersectionAngles[i][j][k-1]) / 2);
						double smid = s0 + r0 * sin( (intersectionAngles[i][j][k]+intersectionAngles[i][j][k-1]) / 2);
			...
			double tmid = t0 + r0 * cos(angle);
			double smid = s0 + r0 * sin(angle);
			bool midPointInside = false;
			for (uint jj = 0 ; jj < tsCircles[i].size();jj++){
				if (j == jj) continue;
			...
			if (debug){
				//fprintf(stdout,"\tARC1 %4d [ %8.3f %8.3f ] \n",insideCount, firstAngle,lastAngle);
			}
	...
	//  For each circle
	//    Sum = 0
 	//    for each part of circle that is uncovered
	//        Sum += Integral
	if (debug){
		cout << endl<<"-->  INTEGRATE OVER ARCS  <--"<<endl<<endl;
	}
	...
	for (uint i = 0; i < _atoms.size();i++){
		...
		for (uint j = 0 ; j < tsCircles[i].size();j++){
			...
			double A = (4*_radii[i]*_radii[i] + tsCircles[i][j][0]*tsCircles[i][j][0] + tsCircles[i][j][1]*tsCircles[i][j][1] + tsCircles[i][j][2]*tsCircles[i][j][2]) / 2;
			...
			for (uint a = 0; a < arcs[i][j].size();a+=2){
				...
				J1  = (abs(beta - alpha) + (tsCircles[i][j][2]*tsCircles[i][j][2] - A)*I1) / 2;
				J2  = (I1 + (tsCircles[i][j][2]*tsCircles[i][j][2] - A)*I2) / 4;
				J3  = (I2 + (tsCircles[i][j][2]*tsCircles[i][j][2] - A)*I3) / 8;


				double radii3 = _radii[i]*_radii[i]*_radii[i];

				// DIRECTLY FROM THE PAPER
				//double volumeIntegral      = (   ( (128*J3*radii3*radii3*_radii[i] ) / 3) + 
				//                                 ( (2  *J1*radii3                  ) / 3) - 
				//                                 ( (8  *J2*radii3*_radii[i]        ) * (3*_atoms(i).getZ()+2*_radii[i]) / 3));

				double volumeIntegral = ((128*J3*radii3*radii3*_radii[i] + 8*J2*radii3*_radii[i]*_radii[i] + 2 *J1*radii3) / 3) -8*radii3*_radii[i]*J2*(_atoms(i).getZ()+_radii[i]);
				//double volumeIntegral   =  (128*J3*radii3*radii3*_radii[i] + 8*J2*radii3*_radii[i]*_radii[i] + 2*J1*radii3)  / 3  -8*radii3*_radii[i]*J2*(_atoms(i).getZ()+_radii[i]);
				//cout << "J1,J2,J3: "<<J1<<","<<J2<<","<<J3<<","<<_radii[i]<<","<<_atoms(i).getZ()<<","<<volumeIntegral<<endl;
				
				if (debug){
					cout << "  T1: "<<128*J3*radii3*radii3*_radii[i]<<endl;
					cout << "  T2: "<<8*J2*radii3*_radii[i]*_radii[i]<<endl;
					cout << "  T3: "<<2*J1*radii3;
					cout << "  T4: "<<8*radii3*_radii[i]*J2*(_atoms(i).getZ()+_radii[i])<<endl;
					cout << "  Sum: "<<((128*J3*radii3*radii3*_radii[i])+(8*J2*radii3*_radii[i]*_radii[i])+(2*J1*radii3))/3;
					cout << "  ALL: "<<((128*J3*radii3*radii3*_radii[i])+(8*J2*radii3*_radii[i]*_radii[i])+(2*J1*radii3))/3 - 8*radii3*_radii[i]*J2*(_atoms(i).getZ()+_radii[i]); 
				}
				...
				if (debug){
					fprintf(stdout, "\tPOS? %4d\n\tI: %8.3f, %8.3f, %8.3f\n\tJ: %8.3f, %8.3f, %8.3f\n\tdelta: %8.3f, %8.3f\n\trr: %8.3f\n",positiveOrientation,I1,I2,I3,J1,J2,J3,sign*2*_radii[i]*_radii[i]*surfaceAreaIntegral,sign*volumeIntegral,(tsCircles[i][j][2]*tsCircles[i][j][2] - A));
				}
				volume      += sign*volumeIntegral;
				surfaceArea += sign*2*_radii[i]*_radii[i]*surfaceAreaIntegral;
			}
		...
		if (positiveOrientation){
			...
			totalSurfaceArea += 4*M_PI*_radii[i]*_radii[i] + surfaceArea;
			totalVolume      += (4/3)*M_PI*_radii[i]*_radii[i]*_radii[i] + volume;

			if (debug){
				fprintf(stdout,"\tVolume subtotal       : %16.3f  %16.3f ===> %16.3f\n",volume,(4/3)*M_PI*_radii[i]*_radii[i]*_radii[i]+volume,totalVolume);
				fprintf(stdout,"\tSurface Area subtotal : %16.3f  %16.3f ===> %16.3f\n",surfaceArea,4*M_PI*_radii[i]*_radii[i] + surfaceArea,totalSurfaceArea);
			}
	...
	for (uint i = 0; i < _atoms.size();i++){
		...
		for (uint j = 0 ; j < tsCircles[i].size();j++){
			...
			for (uint a = 0; a < arcs[i][j].size();a+=2){
				...
				double volumeIntegral = ((128*J3*radii3*radii3*radiiI + 8*J2*radii3*radiiI*radiiI + 2 *J1*radii3) / 3) -8*radii3*radiiI*J2*(_atoms(i).getZ()+radiiI);
				//double volumeIntegral   =  (128*J3*radii3*radii3*radiiI + 8*J2*radii3*radiiI*radiiI + 2*J1*radii3)  / 3  -8*radii3*radiiI*J2*(_atoms(i).getZ()+radiiI);
		...
		if (positiveOrientation){
			...
			atomicVolume      = (4/3)*M_PI*radiiI*radiiI*radiiI + volume;

			if (_debug || debug){
				fprintf(stdout,"\tVolume subtotal       : %16.3f  %16.3f ===> %16.3f\n",volume,(4/3)*M_PI*radiiI*radiiI*radiiI+volume,totalVolume);
