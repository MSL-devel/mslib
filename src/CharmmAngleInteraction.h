...
class CharmmAngleInteraction: public ThreeBodyInteraction {
	...
	public:
		...
		friend ostream & operator<<(ostream &_os, CharmmAngleInteraction & _term) {_os << _term.toString(); return _os;};
...
inline void CharmmAngleInteraction::setParams(vector<double> _params) { if (_params.size() != 2) {cerr << "ERROR 49123: invalid number of parameters in inline void CharmmAngleInteraction::setParams(vector<double> _params)" << endl; exit(49123);} params = _params;}
...
inline string CharmmAngleInteraction::toString() const { char c [1000]; sprintf(c, "CHARMM ANGL %s %s %s %9.4f %9.4f %9.4f %20.6f", pAtoms[0]->toString().c_str(), pAtoms[1]->toString().c_str(), pAtoms[2]->toString().c_str(), params[0], params[1] * 180.0 / M_PI, angle * 180.0 / M_PI, energy); return (string)c; };
//inline unsigned int CharmmAngleInteraction::getType() const {return type;}
inline string CharmmAngleInteraction::getName() const {return typeName;}
