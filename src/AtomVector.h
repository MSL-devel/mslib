...
class AtomVector : public vector<Atom *> {
	public:
		...
		void operator=(const AtomVector & _atoms);
		AtomVector operator+(const AtomVector & _atoms);
		AtomVector operator-(const AtomVector & _atoms);
		void operator+=(const AtomVector & _atoms);
		void operator-=(const AtomVector & _atoms);
		...
		friend ostream & operator<<(ostream &_os, const AtomVector & _av)  {_os << _av.toString(); return _os;};
		...
		double rmsd(const AtomVector &_av) const;
	...
	public:
		...
		void save_checkpoint(string filename) const{
			...
			if (archiveType == "binary"){
				...
				oa << boost::serialization::make_nvp("AtomVector",*this);
			} else {
				std::ofstream fout(filename.c_str());
		...
		void load_checkpoint(string filename){
			...
			if (archiveType == "binary"){
				...
				ia >> boost::serialization::make_nvp("AtomVector",*this);
			} else {
				std::ifstream fin(filename.c_str());
...
// INLINE FUNCTIONS
inline Atom & AtomVector::operator()(size_t _n) { return *((*this)[_n]); }
inline void AtomVector::setName(string _name) { name = _name; }
inline string AtomVector::getName() const { return name;  }
inline CartesianPoint AtomVector::getGeometricCenter() const { return geometricCenter; }
inline string AtomVector::toString() const {
	stringstream ss;
	for (AtomVector::const_iterator k = begin(); k!=end(); k++) {
		ss << **k << endl;
	}
	/*
