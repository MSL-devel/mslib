...
string programName = "clusterHeteroCandidates";
...
void usage() {
	...
	cout << "parfile <file> (contains lines of format \"7162067 08 -8 31 0.75 -0.75 7.6 A 32 31 HA2 7.7 B 38 35 HA1 7.7 B 35 31 HA1 7.8 A 35 35 HA2 7.9\")" << endl << endl;
...
class Structure {
	...
	Structure(string _line, System& _sys, System& _helicalSys) {
		...
		modelNum = toks[0];
		double axialRA = MslTools::toDouble(toks[1]);
		double axialRB = MslTools::toDouble(toks[2]);
		double crossA = MslTools::toDouble(toks[3]);
		double zShiftA = MslTools::toDouble(toks[4]);
		double zShiftB = MslTools::toDouble(toks[5]);
		double xShift = MslTools::toDouble(toks[6]);
		build(axialRA,axialRB,crossA,zShiftA,zShiftB,xShift,_sys,_helicalSys);
	...
	void build(double _axialRotateA, double _axialRotateB, double _crossingAngle, double _zShiftA, double _zShiftB, double _xShift, System & _sys, System & _helicalSys ) {
		...
		Transforms trans;
		trans.translate(chainA, zShiftACP);
		trans.translate(chainB, zShiftBCP);

		//===== Axial Rotation ======
		trans.rotate(chainA, _axialRotateA, origin, zAxis);
		trans.rotate(chainB, _axialRotateB, origin, zAxis);

		//====== Local Crossing Angle ======
		trans.rotate(chainA, (_crossingAngle/2.0), origin, xAxis);
		trans.rotate(axisA,  (_crossingAngle/2.0), origin, xAxis);

		trans.rotate(chainB, (_crossingAngle/2.0), origin, xAxis);
		trans.rotate(axisB,  (_crossingAngle/2.0), origin, xAxis);
		...
		trans.translate(axisB, interDistVect);
		...
		trans.rotate(chainB, m);
		trans.rotate(axisB, m);
